...
### 17. Account for Input without Spaces ([`d0e857c`](https://github.com/Kinsa/ParkingAttendantBackend/commit/d0e857c))
...
- [ ] Should the session window be tracked via a specific datetime object instead of minutes? Minutes work well for a short-term lot. In terms of flexibility, in a long-term lot, someone might purchase a month at a time. The start of the window wouldn't be the time they entered the lot in that case. However, that would probably be tracked in a separate table and we could calculate the number of minutes until expiration to store here.
